...
# 每課題目圖要看多長。單根 K 的課要短（看得清 K 棒），結構課要長（看得到型態）
LESSON_LOOKBACK = {
    "C03": 55,   # 只看突破那一根
    "C10": 55,
    "C05": 90,
...
def build_questions(lesson, pool, prices, n, rng):
    ...
    for _, r in cand.iterrows():
        ...
        lb = LESSON_LOOKBACK.get(lesson.cid, 140)
        q_img = png_b64(lc.render_chart(g, i, {}, mode="plain", lookback=lb,
                                        title=f"{sid}",
                                        subtitle="最後一根 = 突破日 T"))
...
.qhead{display:flex;justify-content:space-between;align-items:baseline;margin-top:30px}
.qno{font-weight:700;font-size:1.05rem}
.meta{color:var(--grey);font-size:.8rem}
.prompt{font-weight:600;margin:10px 0 6px}
.opts{margin:0 0 10px}
.opt{display:block;width:100%;text-align:left;padding:13px 15px;margin:7px 0;border-radius:10px;
  border:1.5px solid var(--line);background:#fff;font-size:.97rem;font-family:inherit;color:var(--ink);cursor:pointer}
.opt.correct{border-color:var(--ok);background:#f0fdf4}
.opt.wrong{border-color:var(--no);background:#fef2f2}
details{border:1px solid var(--line);border-radius:10px;padding:0;margin:10px 0;background:var(--bg)}
details summary{padding:13px 15px;cursor:pointer;font-weight:600;color:var(--blue);list-style:none}
details summary::-webkit-details-marker{display:none}
details summary::after{content:" ▾"}
details[open] summary::after{content:" ▴"}
.reveal{padding:0 15px 14px}
.ansline{font-size:1.02rem;font-weight:700;margin-bottom:6px}
.nums{display:flex;gap:14px;flex-wrap:wrap;margin:8px 0;font-size:.85rem;color:var(--grey)}
...
<hr>
<p class="hint">每題先自己判斷，再展開「顯示答案」對照。若在 Safari 開啟，點選項還會自動計分。</p>
__QUESTIONS__
...
function pick(idx, oi, btn){
  const box = btn.parentElement;
  if(box.dataset.done) return;
  box.dataset.done = '1';
  const a = ANS[idx];
  [...box.children].forEach((b,i)=>{
    if(i===a.answer) b.classList.add('correct');
    else if(i===oi) b.classList.add('wrong');
  });
  document.getElementById('det'+idx).open = true;
  log.push({q:idx+1, lesson:LESSON.cid, stock_id:a.stock_id, date:a.date,
            field:a.field, value:a.value, my_choice:oi, correct:(oi===a.answer)});
  const n=log.length, c=log.filter(r=>r.correct).length;
  document.getElementById('score').textContent =
    `${c} / ${n}　正確率 ${Math.round(c/n*100)}%`;
}
...
def render_questions(qs) -> str:
    """伺服器端就把題目畫好，不依賴 JavaScript。"""
    out = []
    for idx, q in enumerate(qs):
        opts = "".join(
            f'<button class="opt" onclick="pick({idx},{oi},this)">{html.escape(t)}</button>'
            for oi, t in enumerate(q["options"])
        )
        correct_txt = html.escape(q["options"][q["answer"]])
        out.append(f"""
<div class="qhead"><span class="qno">第 {idx+1} 題</span>
  <span class="meta">{html.escape(q['stock_id'])}　{q['date']}</span></div>
<img src="data:image/png;base64,{q['q_img']}" alt="題目圖">
<div class="prompt">{html.escape(q['prompt'])}</div>
<div class="opts" >{opts}</div>
<details id="det{idx}">
  <summary>顯示答案</summary>
  <div class="reveal">
    <div class="ansline">答案：{correct_txt}</div>
    <div>實際 {html.escape(q['field'])} = <b>{q['value']:.2f}</b>
         （本課母體中位數 {q['median']:.2f}）</div>
    <div class="nums">
      <span>後20日 <b>{q['ret20']}</b></span>
      <span>後60日 <b>{q['ret60']}</b></span>
      <span>最大回撤 <b>{q['mae']}</b></span>
    </div>
...
def build_html(lesson, qs, howto, outpath):
    ans = [{"answer": q["answer"], "stock_id": q["stock_id"], "date": q["date"],
            "field": q["field"], "value": q["value"]} for q in qs]
    page = (TEMPLATE
            ...
            .replace("__FORMULA__", html.escape(lesson.formula))
            .replace("__GOOD__", html.escape(lesson.good))
            .replace("__BAD__", html.escape(lesson.bad))
            .replace("__QUESTIONS__", render_questions(qs))
            .replace("__ANS__", json.dumps(ans, ensure_ascii=False))
