...
def build_figure(stock_id, name, daily, wk, segs, dtbl, m, bx, vz,
    ...
    ax.add_patch(Rectangle((n - _bl - .5, bx["lo"]), _bl, bx["hi"] - bx["lo"],
                           facecolor="none", edgecolor="#00838f", ls="--",
                           lw=1.5, zorder=6))
    _gc = {"窄箱": "#2e7d32", "中箱": "#e6a700", "寬箱": "#c0392b"}[bx["grade"]]
    ax.text(n - _bl / 2, bx["hi"] + pad * .06,
            f"{bx['grade']}　箱寬 {bx['width']:.0f}%　位置 {bx['pos']:.0f}% {bx['zone']}",
            ha="center", color=_gc, zorder=7, **zh(10, True))
    # 中線（大腸圈那條）
    _midv = wk["close"].rolling(bx["ma_len"],
                                min_periods=max(2, bx["ma_len"] - 4)).mean()
    ax.plot(range(n), _midv.iloc[base:].to_numpy(float), color="#00838f",
            lw=1.2, ls="-.", zorder=5)
    ax.text(n - .5, bx["mid"], f"  {bx['ma_len']}週中線 {bx['mid']:,.0f}",
            va="center", color="#00838f", zorder=7, **zh(8.5, True))
    ...
    for k, (nm, col, bg) in enumerate(palette):
        ...
        for it in vz[nm][:5]:
            ...
            axb.text(x0 + .045, yy + .006, wr, va="top", color="#222", **zh(8.2))
            yy -= .056 + .050 * wr.count("\n")
