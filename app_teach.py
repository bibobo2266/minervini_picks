...
def verdicts(m: dict, stage_now: int, vcp_ok: bool, vcp_legs) -> dict:
    ...
    mini = [
        f"趨勢模板 {m['tt_n']}/7 條符合（RS 排名需全母體，本圖不計）",
...
def build_figure(stock_id, name, daily, wk, segs, dtbl, m, vz,
                 vcp_legs, vcp_ok=False, show_verdict=True, show_deduct=True,
    ...
    # 主圖
    gsl = gs[1, 0].subgridspec(2, 1, height_ratios=[.78, .22], hspace=.05)
    ax = fig.add_subplot(gsl[0]); axv = fig.add_subplot(gsl[1], sharex=ax)
    ...
    ax.annotate(f"52 週高點 {m['hi52']:,.0f}", xy=(_hx, m["hi52"]),
                xytext=(max(_hx - n * .12, n * .05), m["hi52"] + pad * .55),
    ...
    for i, d0 in enumerate(view["date"]):
        ...
        if key not in seen and i % 2 == 0:
            seen.add(key); ticks.append(i)
            labs.append(f"{d0:%Y/%m}" if d0.month == 1 or i == 0 else f"{d0:%m}月")
    axv.set_xticks(ticks); axv.set_xticklabels(labs, **zh(8))

    # 右上：日線放大
    gsr = gs[1, 1].subgridspec(2, 1, height_ratios=[.44, .56], hspace=.20)
    ...
    axd.text(len(dd) * .02, m["pivot"] * 1.004, f"Pivot {m['pivot']:,.0f}",
             color="#7e57c2", **zh(8.5, True))
    dlo, dhi = float(dd["low"].min()), float(dd["high"].max())
    dp = (dhi - dlo) * .12
    axd.set_ylim(dlo - dp, dhi + dp)
    axd.grid(alpha=.22, ls=":"); axd.tick_params(labelsize=7.5)
    dt_ticks = [i for i in range(0, len(dd), 21)]
    axd.set_xticks(dt_ticks)
    axd.set_xticklabels([f"{dd['date'].iloc[i]:%m}月" for i in dt_ticks], **zh(8))
    axd.set_title("日線放大圖（近 3 個月）", color="#1f4e9c", pad=6, **zh(12, True))
    for s_ in axd.spines.values():
        s_.set_color("#1f4e9c")

    # 右下：扣抵值
    axk = fig.add_subplot(gsr[1]); axk.axis("off")
    axk.add_patch(FancyBboxPatch((0, 0), 1, 1, boxstyle="round,pad=.012",
                  transform=axk.transAxes, fc="#fffaf3", ec="#e07b1a", lw=1.6))
    axk.text(.5, .95, f"扣抵值怎麼看？（以 {ded_period} 期均線為例）",
             ha="center", color="#d2691e", **zh(12.5, True))
    if show_deduct and not dtbl.empty:
        vals = [f"{v:,.0f}" for v in dtbl["扣抵值"].tolist()[:5]] + [f"{m['price']:,.0f}"]
        labs2 = [str(i + 1) for i in range(len(vals) - 1)] + ["最新"]
        axk.text(.5, .875, f"{ded_period} 期均線扣抵值示意（左=最舊，將被移出）",
                 ha="center", color="#666", **zh(8.5))
        cw = .86 / len(vals); x0 = .07
        for j, v in enumerate(vals):
            for r, yy in ((0, .795), (1, .735)):
                txt = labs2[j] if r == 0 else v
                axk.add_patch(Rectangle((x0 + j * cw, yy - .028), cw, .056,
                              fc="#fdf6e8" if r == 0 else "white",
                              ec="#c9a227", lw=.8, transform=axk.transAxes))
                axk.text(x0 + j * cw + cw / 2, yy, txt, ha="center", va="center",
                         color="#c0392b" if (r == 1 and j == len(vals) - 1) else "#333",
                         **zh(8.5))
        for j in range(len(vals)):
            ec = "#c0392b" if j == 0 else ("#2e7d32" if j == len(vals) - 1 else "#999")
            axk.add_patch(Rectangle((x0 + j * cw + .004, .565), cw - .008, .058,
                          fc="white", ec=ec, ls="--" if j == 0 else "-", lw=1.3,
                          transform=axk.transAxes))
            axk.text(x0 + j * cw + cw / 2, .594, vals[j], ha="center", va="center",
                     color=ec if j in (0, len(vals) - 1) else "#444",
                     **zh(8.5, j in (0, len(vals) - 1)))
        axk.text(x0 + cw / 2, .485, "最舊\n→ 移出", ha="center", va="top",
                 color="#c0392b", **zh(7.5))
        axk.text(x0 + (len(vals) - .5) * cw, .485, "最新\n加入", ha="center",
                 va="top", color="#2e7d32", **zh(7.5))
    for i, (t1, c1) in enumerate([("現價 > 扣抵值 → 均線較容易 上彎", "#c0392b"),
                                  ("現價 ≈ 扣抵值 → 均線大致走平", "#555"),
                                  ("現價 < 扣抵值 → 均線支撐 轉弱", "#1f7a3d")]):
        axk.text(.075, .355 - i * .072, t1, color=c1, **zh(10, True))
    axk.add_patch(Rectangle((.045, .055), .91, .085, fc="#f6a623", alpha=.30,
                  ec="#e07b1a", lw=1, transform=axk.transAxes))
    axk.text(.5, .098, "扣抵值不是預測股價，而是看均線未來幾期的數學結構。",
             ha="center", va="center", color="#8a4b08", **zh(9, True))
    ...
    for k, (nm, col, bg) in enumerate(palette):
        ...
        for it in vz[nm][:4]:
            ...
            axb.text(x0 + .045, yy + .006, wr, va="top", color="#222", **zh(8.8))
            yy -= .072 + .060 * wr.count("\n")
...
with st.expander("趨勢模板逐條檢查（Minervini 前 7 條）"):
    st.dataframe(pd.DataFrame({"條件": list(m["tt"].keys()),
                               "符合": list(m["tt"].values())}),
                 use_container_width=True, hide_index=True)
    st.caption("第 8 條（RS 排名 ≥ 70）需要全母體排序，請看 Minervini app。")

with st.expander("階段分段明細"):
    st.dataframe(pd.DataFrame(
        [{"起": wk['date'].iloc[a].date(), "迄": wk['date'].iloc[b].date(),
          "週數": b - a + 1, "階段": STAGE_NAME.get(s_, s_)} for a, b, s_ in segs]),
        use_container_width=True, hide_index=True)
